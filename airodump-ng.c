...
void RunShellScript()
{
   ...
   //child
   char* env[2];
   char strEnvEssid[sizeof(G.pSelectedAP->essid)+strlen("ESSID=")];
   snprintf(strEnvEssid, sizeof(strEnvEssid), "ESSID=%s", G.pSelectedAP->essid);
   env[0] = strEnvEssid;
   env[1] = NULL;
...
#define KEY_e           0x65    //start user defined shell script
...
void input_thread(void *arg)
{
	...
	while (G.do_exit == 0)
	{
		...
		if ((keycode == KEY_e)&&(G.pSelectedAP))
		   RunShellScript();
